<template>
  <div class="container">
    <div class="header-flash">
      <h1>
        <i class="fas fa-globe" />Szukaj  zestawów
        <div class="search">
          <input
            v-model="search"
            type="text"
          ...
          <i
            class="fas fa-search"
            style="background: #92589E; padding-left: 5px;"
          />
        </div>
      </h1>
    </div>
  ...
  methods: {
    load() {
      ...
      if (this.search.length > 2) {
        formData.append('topic', this.search);
        formData.append('title', this.search);

        fetch('http://127.0.0.1/api/flashcard/readSearch.php',{
          method: 'POST',
          body: formData
        })
        ...
        fetch('http://127.0.0.1/api/flashcard/read.php',{
          method: 'POST',
          body: formData
        })
        .then(response=> response.json())
